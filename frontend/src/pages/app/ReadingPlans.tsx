...
import { readingPlansService } from '@/services/readingPlans.service'
...
export function ReadingPlans() {
  ...
  const { data: progressRows = [] } = useQuery({
    queryKey: ['reading-plan-progress'],
    queryFn: readingPlansService.listProgress,
  })

  const { mutate: startPlan } = useMutation({
    mutationFn: readingPlansService.startPlan,
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['reading-plan-progress'] }),
  })

  const progressByPlan = new Map(progressRows.map((p) => [p.planId, p]))
    ...
    <div className="mx-auto max-w-5xl px-4 py-8 md:px-8 space-y-8">
      ...
      <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
        {filteredPlans.map((plan) => {
          ...
          const isCurrent = progress?.isCurrent ?? false
                ...
                {isCurrent ? (
                  <div>
                    <div className="flex items-center justify-between text-xs font-semibold text-indigo-700 mb-1.5">
                      ...
                      <span className="font-serif font-bold text-purple-700">{progress?.progressPercent ?? 0}%</span>
                    </div>
